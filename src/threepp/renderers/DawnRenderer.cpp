...
struct DawnRenderer::Impl {
    ...
    TextureEntry& getOrCreateTexture(Texture* tex) {
        ...
        // Convert RGB to RGBA if needed (WebGPU requires RGBA8)
        std::vector<unsigned char> rgbaData;
        const unsigned char* uploadPtr = data.data();
        size_t uploadSize = data.size();
        bool isRGB = (tex->format == Format::RGB) || (data.size() == static_cast<size_t>(w) * h * 3);
        if (isRGB) {
            rgbaData.resize(static_cast<size_t>(w) * h * 4);
        ...
        uint32_t unpaddedBytesPerRow = w * 4;
        ...
        if (paddedBytesPerRow == unpaddedBytesPerRow) {
            wgpuQueueWriteTexture(queue, &dst, uploadPtr, uploadSize, &layout, &extent);
        } else {
            // Need row padding
            size_t paddedSize = static_cast<size_t>(paddedBytesPerRow) * h;
            std::vector<unsigned char> padded(paddedSize, 0);
            for (unsigned int row = 0; row < h; row++) {
                std::memcpy(padded.data() + row * paddedBytesPerRow,
                            uploadPtr + row * unpaddedBytesPerRow,
                            unpaddedBytesPerRow);
            }
            wgpuQueueWriteTexture(queue, &dst, padded.data(), paddedSize, &layout, &extent);
        }
...
@fragment fn fs_main() {}
    ...
    void renderCustomShaderObject(WGPURenderPassEncoder pass, Mesh* mesh,
        ...
        if (needRebuild) {
            ...
            // Discover float/int uniforms to determine if we need a custom UBO
            ...
            if (hasCustomUniforms) {
                // We'll pack all scalar uniforms into a buffer at binding 2
                // For now, allocate a generous 256 bytes
                WGPUBindGroupLayoutEntry e{}; e.binding = 2;
                e.visibility = WGPUShaderStage_Vertex | WGPUShaderStage_Fragment;
                e.buffer.type = WGPUBufferBindingType_Uniform;
            ...
            // Sorted by name for deterministic binding order
        ...
        // Allocate custom transform/light buffers if needed
        ...
        // Copy the already-uploaded light data
        // lightBuffer is already written by uploadLightDataWorldSpace() in render()
        // We can just point to lightBuffer directly in the bind group.
        ...
        if (hasCustomUniforms) {
            // Pack uniforms into 256 byte buffer
            float uboData[64]; // 256 bytes
    ...
    void renderObject(WGPURenderPassEncoder pass, Object3D& object,
        ...
        if (auto mesh = object.as<Mesh>()) {
            ...
            if (geometry && geometry->hasAttribute("position")) {
                ...
                // Check for ShaderMaterial first — custom shader path
                if (auto sm = dynamic_cast<ShaderMaterial*>(rawMat)) {
                    if (!sm->vertexShader.empty() && !sm->fragmentShader.empty()) {
